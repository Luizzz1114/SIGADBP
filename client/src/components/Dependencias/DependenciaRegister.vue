...
<template>
  <Drawer v-model:visible="visible" :dismissable="false" position="right" class="w-full! md:w-180!">
    ...
    <Form id="form-registrar-dependencia" v-slot="$form" :resolver="resolver" :initialValues="dependencia" @submit="onFormSubmit" class="flex flex-col">
    ...
    <template #footer>
      <div class="flex justify-end gap-3 p-3! border-t border-slate-200 dark:border-slate-700 w-full">
        ...
        <Button label="Registrar" type="submit" form="form-registrar-dependencia" />
      </div>
    </template>
  </Drawer>
</template>