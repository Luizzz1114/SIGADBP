...
const camposHabilitados = computed(() => {
  ...
  return {
    ...
    presupuesto: m === 'Compra'
  };
});

const onChangeMotivo = (e) => {
  const m = e.value;
  incorporacion.value.motivo = m;
...
const onFormSubmit = (e) => {
  ...
  const payload = {
    ...e.values,
    id: props.incorporacion.id,
    dependencia: props.incorporacion.idd,
    responsable: props.incorporacion.idp,
...
watch(visible, async(isOpen) => {
  if (isOpen) {
    incorporacion.value = {
      ...initialFormState,
      ...props.incorporacion,
      motivo: props.incorporacion.motivo || '',
      dependencia: props.incorporacion.idd,
      bienes: []
    };
    ...
    bienesSeleccionados.value = bienesActuales.map(b => ({
      ...
      id: b.id,
      gasto: b.gasto || 0,
      id_presupuesto: b.idpresupuesto || null
    }));
    ...
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col">
          <span class="font-bold text-lg! dark:text-slate-50">Actualizar incorporación</span>
        </div>
      </div>
    </template>
    <Form id="form-actualizar-incorporacion" v-slot="$form" :initialValues="incorporacion" :resolver="resolver" :key="incorporacion?.id" @submit="onFormSubmit" class="flex flex-col">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 mt-4">
        <div class="flex flex-col gap-1">
          ...
          <Select :model-value="{ nombre: props.incorporacion.dependencia, responsable: props.incorporacion.responsable, cedula: props.incorporacion.cedula }" disabled readonly dropdownIcon="false" size="small" fluid>
        ...
        <div class="flex flex-col gap-1">
          ...
          <DatePicker
            name="fecha_entrada"
            :model-value="$form.fecha_entrada?.value"
