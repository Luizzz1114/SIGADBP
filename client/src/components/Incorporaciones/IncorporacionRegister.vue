...
<template>
  <Drawer v-model:visible="visible" position="right" :dismissable="false" class="w-full! md:w-212!">
    ...
    <Form id="form-registrar-incorporacion" v-slot="$form" :initialValues="incorporacion" :resolver="resolver" @submit="onFormSubmit" class="flex flex-col">
    ...
    <template #footer>
      <div class="flex justify-end gap-3 p-3! border-t border-slate-200 dark:border-slate-700 w-full">
        ...
        <Button label="Registrar" type="submit" form="form-registrar-incorporacion" />
      </div>
    </template>
  </Drawer>
</template>