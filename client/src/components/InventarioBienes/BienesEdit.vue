  ...
  <Drawer v-model:visible="visible" position="right" :dismissable="false" class="w-full! md:w-180!">
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div class="flex flex-col">
          <span class="font-bold text-lg! dark:text-slate-50">Actualizar bien</span>
        </div>
      </div>
    </template>
    <Form id="form-actualizar-bien" v-slot="$form" :initialValues="bien" :resolver="resolver" @submit="onFormSubmit">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 mt-4">
        ...
        <div class="flex flex-col gap-1">
          ...
          <Select name="categoria" :options="categorias" placeholder="Selecione" dropdownIcon="false" size="small" fluid disabled />
      ...
      <div v-if="!$form.categoria?.value" class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 mt-4">
        <div class="flex flex-col col-span-full py-2 px-4 gap-1 rounded-lg bg-slate-100 dark:bg-slate-800">
      ...
      <div v-else-if="$form.categoria?.value === 'Tecnológico'" class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 mt-4">
        ...
        <div class="flex flex-col gap-1">
          ...
          <InputText name="especificaciones" maxlength="50" id="especificaciones" autocomplete="off" size="small" fluid />
    ...
    <template #footer>
      <div class="flex justify-end gap-3 p-3! border-t border-slate-200 dark:border-slate-700 w-full">
        ...
        <Button label="Actualizar" type="submit" form="form-actualizar-bien" />
      </div>
    </template>
  </Drawer>
</template>